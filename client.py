...
bombs = [Pulesos(randint(-2000, 2000), randint(-2000, 2000), 10,
           (0, 0, 0))
       for _ in range(300)]
...
while running:
   ...
   for bomb in bombs:
       if bomb.check_collision(my_player[0], my_player[1], my_player[2]):
           to_remove_bomb.append(bomb)
           my_player[2] -= int(bomb.radius * 0.2)
       else:
           sx = int((bomb.x - my_player[0]) * scale + 500)
           sy = int((bomb.y - my_player[1]) * scale + 500)
           draw.circle(window, bomb.color, (sx, sy), int(bomb.radius * scale))

   for bomb in to_remove_bomb:
       bombs.remove(bomb)
